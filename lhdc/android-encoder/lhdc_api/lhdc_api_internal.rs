...
pub enum Error {
    ...
    InvalidInputParam,
    InvalidVersion(u32),
    Context(crate::enc::context::Error),
    Process(crate::enc::process::Error),
}

pub type Result<T> = std::result::Result<T, Error>;

impl From<crate::enc::context::Error> for Error {
    fn from(err: crate::enc::context::Error) -> Self {
        Self::Context(err)
    }
}

impl From<crate::enc::process::Error> for Error {
    fn from(err: crate::enc::process::Error) -> Self {
        Self::Process(err)
    }
}
...
pub fn lhdcv5_encoder_encode(
    ...
    *written_bytes = 0;
    ...
    let out_tmp = lhdc.enc_out_buf.as_mut().as_mut_bytes();
