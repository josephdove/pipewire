...
#[derive(Debug)]
pub enum Error {
    Arith(crate::arith::Error),
    BitOutputter(crate::bits::output::Error),
    TargetRange,
    BadParams,
...
impl From<crate::arith::Error> for Error {
    fn from(err: crate::arith::Error) -> Self {
        Self::Arith(err)
    }
}

impl From<crate::bits::output::Error> for Error {
    fn from(err: crate::bits::output::Error) -> Self {
        Self::BitOutputter(err)
    }
}
...
fn coef_calc(level: &mut fdata_buffer_struct, init_bn: i32, enc_pixel_size_byte: usize) {
    ...
    let coef7 = <[i32]>::mut_from_bytes(level.fdata_q_quant_2_symbol.as_mut().as_mut_bytes()).unwrap();
...
fn lhdc_enc_symbol_calc(enc_text: &mut fdata_all_buffer_struct) -> i32 {
    let umem8 = enc_text.fdata_enc_buffer.level.fdata_q_quant_bit_num.as_ref().as_bytes();
...
fn lhdc_enc_nbyte_out(
    ...
    let lossy_uc = enc_text.fdata_enc_buffer.level.fdata_q_quant_bit_num.as_ref().as_bytes();
    let umem8: &mut [u8] =
        &mut enc_text.fdata_enc_buffer.level.fdata_q_quant_2_symbol.as_mut().as_mut_bytes()
...
fn lhdc_enc_nbyte_esti(enc_text: &mut fdata_all_buffer_struct) -> i32 {
    ...
    let real_data: &[u32] =
        &<[u32]>::ref_from_bytes(enc_text.fdata_enc_buffer.level.fdata_q_quant.as_ref().as_bytes()).unwrap()
    ...
    let real_data: &[u32] = &<[u32]>::ref_from_bytes(level.fdata_q_quant.as_ref().as_bytes()).unwrap()
        [enc_text.pixel_num as usize - level.valid as usize..];

    let mut total: u32 = 0;
    if level.symbol_flag == 1 {
        // coef7, doesn't alias rb, but does alias queue1
        let coef = <[i32]>::mut_from_bytes(level.fdata_q_quant_2_symbol.as_mut().as_mut_bytes()).unwrap();
    ...
    enc_text.fdata_enc_buffer.level.fdata_q_size = lhdc_enc_symbol_generation(
        ...
        level.fdata_q_quant_bit_num.as_mut().as_mut_bytes(),
...
fn lhdc_enc_lossy_freq_process(fdata_all_buffer: &mut fdata_all_buffer_struct, channel: i32) {
    let data_in = <[i32]>::mut_from_bytes(
        fdata_all_buffer.frequency_mem.frequency_buffers.fft_coef_in.as_mut().as_mut_bytes(),
    )
    .unwrap();
...
pub fn lhdc_enc_encode(
    ...
    let ch0_bytes = <[u32] as IntoBytes>::as_bytes(&ch0[..]);
    output_buffer_top[0..copy_len].copy_from_slice(&ch0_bytes[0..copy_len]);
    if fdata_all_buffer.ch_num >= 2_i32 {
        let ch1_bytes = <[u32] as IntoBytes>::as_bytes(&ch1[..]);
        output_buffer_top[copy_len..copy_len * 2].copy_from_slice(&ch1_bytes[0..copy_len]);
    }
    *output_frame_size =
